...
def exec_stmt(stmt, env):
    ...
    if t == 'let':
        ...
        result = current + val if op == '+' else current - val
        ...
        if is_true(eval_expr(stmt[1], env)):
        ...
        raise ReturnEx(eval_expr(stmt[1], env) if len(stmt) > 1 else None)

    elif t == 'func':
        def f(*args):
            f_env = Env(env)
            for pname, pval in zip(stmt[2], args):
                f_env.define_var(pname, pval)
            try:
                exec_stmt(stmt[3], f_env)
            except ReturnEx as r:
                return r.val
        env.define_func(stmt[1], f)

    elif t == 'class':
        env.define_class(stmt[1], stmt)

    elif t == 'try':
        try:
            exec_stmt(stmt[1], env)
        except (BreakEx, ContinueEx, ReturnEx):
            raise
        except Exception:
            if len(stmt) > 2:
                exec_stmt(stmt[2], env)
